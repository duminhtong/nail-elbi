...
export default function NailboxPage() {
  ...
  return (
    <PageContainer>
      ...
      {/* Banner / Intro */}
      <div className="mb-20 bg-neu/20 border border-border-soft/10 rounded-2xl p-8 md:p-12 max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10">
        <div className="flex-1 space-y-6">
          <h3 className="font-display text-2xl md:text-3xl font-light text-ink uppercase tracking-tight">
        ...
        <div className="w-full md:w-2/5 aspect-[4/3] rounded-xl overflow-hidden shadow-premium-sm border border-border-soft/10">
          <img 
      ...
      <div className="mb-24">
        <div className="flex items-center justify-between mb-12 max-w-5xl mx-auto border-b border-border-soft/10 pb-4">
          <h3 className="font-display text-xl font-medium text-ink uppercase tracking-wider">CÁC MẪU THIẾT KẾ</h3>
          <span className="text-xs text-muted font-light uppercase tracking-widest">{items?.length || 0} Thiết kế</span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-5xl mx-auto">
            {items?.map((item) => (
              <Card key={item.id} className="h-full border border-border-soft/10 p-0 shadow-premium-sm hover:shadow-premium transition-all duration-500 rounded-2xl bg-white flex flex-col overflow-hidden group">
                {/* Image Showcase */}
                <div className="aspect-[4/3] relative overflow-hidden shrink-0 bg-neu">
                  <img 
                  ...
                  />
                  {item.is_custom && (
                    <div className="absolute top-6 left-6 bg-rose/90 backdrop-blur-md px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest text-white shadow-premium-sm">
                      Custom Thiết Kế
                    </div>
                  )}
                </div>
                ...
                <CardContent className="p-8 flex-1 flex flex-col">
                  ...
                  <div className="flex justify-between items-start gap-4 mb-3">
                    <h4 className="font-display text-lg font-medium text-ink group-hover:text-rose transition-colors line-clamp-1 leading-tight tracking-tight uppercase">
                      {item.name}
                    </h4>
                  </div>
                  ...
                  {/* Description */}
                  <p className="text-muted text-xs line-clamp-3 mb-6 leading-relaxed font-light flex-1">
                  ...
                  <div className="space-y-3 mb-8 border-t border-border-soft/10 pt-4 text-xs font-light text-muted">
                    <div className="flex justify-between">
                      <span className="uppercase tracking-wider">Sizes sẵn có:</span>
                      <span className="font-medium text-ink">{item.sizes.join(', ')}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="uppercase tracking-wider">Forms hỗ trợ:</span>
                      <span className="font-medium text-ink">{item.forms.join(', ')}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-auto">
                    <Button 
                      variant="outline"
                      onClick={scrollToSizing}
                      className="border-border-soft/20 text-ink text-[10px] font-bold h-10 uppercase tracking-wider rounded-lg"
                    >
                      Đo Size Tay
                    </Button>
                    <Button 
