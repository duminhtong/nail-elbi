...
export default function GelXWorkshopClient() {
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowRight') setActive((current) => current === null ? 0 : (current + 1) % shots.length)
      if (event.key === 'ArrowLeft') setActive((current) => current === null ? shots.length - 1 : (current - 1 + shots.length) % shots.length)
    }
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.wrap}>
        ...
        <section className={styles.hero} aria-labelledby="workshop-title">
          <div className={styles.heroCopy}>
            <p className={styles.kicker}><span className={styles.dot} /> Workshop / Gel X</p>
            <h1 id="workshop-title"><span>GEL X</span><span>1 NGÀY</span></h1>
            <div className={styles.claim}><i aria-hidden="true" />HỌC LÀ ỨNG DỤNG ĐƯỢC SALON</div>
            <p className={styles.lead}>Một ngày tập trung vào nền tảng, độ chính xác và những kỹ thuật có thể mang thẳng vào nhịp làm việc thực tế tại salon.</p>
            <div className={styles.heroActions}>
              <a href="#gallery" className={styles.primaryButton}>Xem ảnh kỹ thuật <span>↓</span></a>
              <a href="#curriculum" className={styles.secondaryButton}>Nội dung workshop <span>↓</span></a>
            </div>
          </div>
          <button className={styles.heroImage} onClick={() => setActive(0)} aria-label="Mở ảnh chính Gel X">
            <Image src={shots[0].src} alt="Kỹ thuật Gel X tại Nail Elbi" fill priority sizes="(max-width: 900px) 100vw, 48vw" />
        ...
        <section id="curriculum" className={styles.section} aria-labelledby="curriculum-title">
          ...
          <div className={styles.contentGrid}>
            <div className={styles.panel}>{curriculum.map((group, index) => <div className={styles.curriculumRow} key={group.title}><span className={styles.number}>0{index + 1}</span><div><h3>{group.title}</h3><ul>{group.items.map((item) => <li key={item}>{item}</li>)}</ul></div></div>)}</div>
            <div className={`${styles.panel} ${styles.impactPanel}`}><p className={styles.impact}>ÚP KHÔNG MÀI GỜ<br />KHÔNG BÙ CỨNG MÓNG</p><div className={styles.spec}><b>THỜI GIAN</b>1 NGÀY</div><div className={styles.spec}><b>MỤC TIÊU</b>ỨNG DỤNG THỰC CHIẾN SALON</div><div className={styles.signature}><span>Người hướng dẫn</span><strong>Kim Ngân Lê</strong></div></div>
          </div>
        </section>
      ...
      {active !== null && <div className={styles.lightbox} role="dialog" aria-modal="true" aria-label="Xem ảnh kỹ thuật" onClick={(event) => { if (event.target === event.currentTarget) setActive(null) }}><button ref={closeButtonRef} className={styles.close} onClick={() => setActive(null)} aria-label="Đóng ảnh">×</button><button className={`${styles.navButton} ${styles.prev}`} onClick={() => setActive((active - 1 + shots.length) % shots.length)} aria-label="Ảnh trước">←</button><figure className={styles.lightboxFigure}><Image src={shots[active].src} alt={shots[active].title} fill sizes="90vw" priority /><figcaption>{shots[active].label} / {shots[active].title}</figcaption></figure><button className={`${styles.navButton} ${styles.next}`} onClick={() => setActive((active + 1) % shots.length)} aria-label="Ảnh tiếp theo">→</button></div>}
    </main>
  )
}
